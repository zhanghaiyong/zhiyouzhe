...
//#import "ZZPhotoHud.h"
@interface ZZPhotoPickerViewController ()<UICollectionViewDelegate,UICollectionViewDataSource,UICollectionViewDelegateFlowLayout,ZZPhotoPickerCellDelegate>
...
- (UIBarButtonItem *)backBtn{
    if (!_backBtn) {
        ...
        [button setTitleColor:lever1Color forState:UIControlStateNormal];
...
-(UIButton *)doneBtn{
    if (!_doneBtn) {
        _doneBtn = [[UIButton alloc]initWithFrame:CGRectMake(0, 0, SCREEN_WIDTH, 44)];
        ...
        [_doneBtn setTitleColor:lever1Color forState:UIControlStateNormal];
    }
    return _doneBtn;
}
...
//    [view addSubview:self.previewBtn];
    ...
    viewLine.backgroundColor = backgroudColor;
...
-(void)setupCollectionViewUI
{
    ...
    _picsCollection.backgroundColor = backgroudColor;
...
{
    UIButton *button = cell.selectBtn;
...
//
//    ZZPhotoPickerCell *cell = (ZZPhotoPickerCell *)[self collectionView:collectionView cellForItemAtIndexPath:indexPath];
//    [self selectPicBtn:cell.selectBtn];
//}
...
-(void)showSelectPhotoAlertView:(NSInteger)photoNumOfMax
{
    if (IOS8_OR_LATER) {
        ...
        UIAlertView *alert = [[UIAlertView alloc]initWithTitle:@"提醒" message:[NSString stringWithFormat:Alert_Max_Selected,(long)photoNumOfMax] delegate:self cancelButtonTitle:nil otherButtonTitles:@"确定", nil];
        [alert show];
    }
